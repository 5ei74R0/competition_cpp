...
template<class T>
std::vector<T> Dijkstra(const GraphWeighted<T> &graph, const std::int32_t start_vertex) {
    constexpr auto infinity = std::numeric_limits<T>::max();
    std::vector<T> min_dist(graph.size(), infinity);

    using P = std::pair<T, std::int32_t>;  // std::pair< DistFromStartingVertex, IndexOfVertex >
