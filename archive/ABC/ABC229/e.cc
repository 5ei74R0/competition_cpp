/**
 * @file e.cc
 * @brief solved https://atcoder.jp/contests/abc229/tasks/abc229_e
 */
...
class DisjointSetUnion {
 public:
  using i32 = int32_t;
  explicit DisjointSetUnion(i32 sz) : parent(sz), siz(sz, 1), group_num(sz) {
    for (i32 i = 0; i < sz; ++i) parent[i] = i;
  }

  // merge two groups
  i32 add_new_node() {
    parent.push_back(siz.size());
    siz.push_back(0);
    ++group_num;
    return node_num() - 1;
  }
  ...
  bool is_same(i32 x, i32 y) { return (root(x) == root(y)); }
  // Return size of the group including x
  i32 size(i32 x) { return siz[root(x)]; }

  bool has_node(i32 i) { return 0 <= i && i < parent.size(); }

  i32 count() { return group_num; }

  i32 node_num() { return parent.size(); }
...
void solve() {
  ...
  // See the operation by reverse order (See the operation as "merge")
  vi answer_container = {0};
  auto dsu = DisjointSetUnion(0);
  auto graph2dsu = std::unordered_map<i32, i32>();

  for (i32 i = N - 1; i >= 1; --i) {
    i32 new_id = dsu.add_new_node();
    graph2dsu[i] = new_id;
    for (const auto& neighbor : graph[i]) {
      if (graph2dsu.find(neighbor) != graph2dsu.end()) {
        dsu.merge(graph2dsu[i], graph2dsu[neighbor]);
      }
    }
    answer_container.emplace_back(dsu.count());
  }
  std::reverse(all(answer_container));
  print<'\n'>(answer_container);
}
};  // namespace competition
...
constexpr i32 inf32 = max32 - i32(u32(~0) >> 3);
