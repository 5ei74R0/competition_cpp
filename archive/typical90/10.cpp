...
signed main() {
    ...
    usize n; std::cin >> n;
    auto csum1 = make_vec(n+1, 0);
    auto csum2 = make_vec(n+1, 0);
    i32 c, p;
    rep (i,n) {
        std::cin >> c >> p;
        csum1[i+1] += csum1[i] + (c == 1 ? p : 0);
        csum2[i+1] += csum2[i] + (c == 2 ? p : 0);
    }
    ...
    while (q--) {
        std::cin >> l >> r;
        std::cout << csum1[r] - csum1[l-1] << ' ';
        std::cout << csum2[r] - csum2[l-1] << '\n';
    }
}
